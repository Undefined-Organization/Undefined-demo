...
import { AddVips } from "../../../api/vips";
class vipAdd extends Component {
    constructor() {
        super()
        this.state = {
            name: '',
    ...
    submit = () => {
        let { name, img, vipType, desc } = this.state
        if (!this.state.img) return message.info('请先上传图片')
        AddVips(name, img, vipType, desc)
            .then((res) => {
                console.log(res)
                message.success('添加成功', 1)
                window.location.href = 'http://localhost:3000/#/admin/vip/list'
            })
            .catch((err) => {
    ...
    render() {
        ...
        return (
            <div style={{ fontSize: '16px', fontWeight: '550' }}>
                ...
                <Select style={{ display: 'inline-block', width: '300px', }} onChange={this.handleChange} defaultValue="青铜会员" style={{ width: 120, margin: '0 0 5px 0' }} >
